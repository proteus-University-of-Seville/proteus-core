...
>
    ...
    <xsl:template match="booleanProperty">
        <xsl:choose>
            <xsl:when test="current()/text() = 'true'">
                <xsl:value-of select="proteus-utils:i18n('xslt.yes')"/>
            </xsl:when>
            <xsl:otherwise>
                <xsl:value-of select="proteus-utils:i18n('xslt.no')"/>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:template>
    ...
    <xsl:template match="fileProperty">
        ...
        <div>
            <xsl:choose>
                ...
                <xsl:otherwise>
                    <span class="tbd"><xsl:value-of select="proteus-utils:i18n('xslt.tbd_expanded')"/></span>
                </xsl:otherwise>
            </xsl:choose>
        </div>
    </xsl:template>
